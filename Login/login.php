<?php
    ...
    $quer = mysqli_query($konek, "SELECT * FROM login_user");
    $idd = mysqli_fetch_assoc($quer);
    if(isset($_POST['login'])){
        $username=$_POST['username'];
        $password=$_POST['password'];
        $query= mysqli_query($konek, "SELECT * FROM login_user WHERE username='$username' AND password='$password'");
        if(mysqli_num_rows($query)>0){
          $id = mysqli_fetch_assoc($query);
            $_SESSION['user'] = $_POST['username'];
            $_SESSION['id_user'] = $id['id'];
            $_SESSION['nama'] = $id['nama'];
            $_SESSION['kerja'] = $id['kerja'];
            $_SESSION['alamat'] = $id['alamat'];
            $_SESSION['nohp'] = $id['nohp'];
...
<div class="login-page">
  <div class="form">
    ...
    <form class="login-form">
      <button type="button" onclick="window.location.href='signup.php'">Dont have an account?SIGN UP</button>
    </form>
  </div>
</div>
